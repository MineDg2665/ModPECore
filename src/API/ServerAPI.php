...
class ServerAPI{
	...
	public function load(){
		...
		if(!file_exists(DATA_PATH . "mods/")){
			mkdir(DATA_PATH . "mods/", 0777);
		}
		//Init all the events
		...
		$this->config = new Config(DATA_PATH . "server.properties", CONFIG_PROPERTIES, [
			...
			"description" => "Server made using ModPECore",
		...
		console("[INFO] This server is running ModPECore version " . MAJOR_VERSION . FORMAT_RESET . " \"" . CODENAME . "\" (MCPE: " . CURRENT_MINECRAFT_VERSION . ") (API " . CURRENT_API_VERSION . ") (PHP " . PHP_VERSION . ")", true, true, 0);
		console("[INFO] ModPECore is distributed under the LGPL License", true, true, 0);
	...
	private function loadProperties(){
		if(($memory = str_replace("B", "", strtoupper($this->getProperty("memory-limit")))) !== false){
			...
			if($real < 128){
				console("[WARNING] ModPECore may not work right with less than 128MB of RAM", true, true, 0);
			}
