...
class PocketMinecraftServer{
	...
	private function load(){
		...
		/*if(defined("DEBUG") and DEBUG >= 0){
			@cli_set_process_title("ModPECore ".MAJOR_VERSION);
		}*/
		...
		$this->extraprops = new Config(DATA_PATH . "extra.properties", CONFIG_PROPERTIES, [
			...
			"discord-bot-name" => "ModPECore Logger",
			"despawn-mobs" => true, 
	...
	public function titleTick(){
		$time = microtime(true);
		if(defined("DEBUG") and DEBUG >= 0){
			//echo "\x1b]0;ModPECore " . MAJOR_VERSION . " | Online " . count($this->clients) . "/" . $this->maxClients . " | RAM " . round((memory_get_usage() / 1024) / 1024, 2) . "MB | U " . round(($this->interface->bandwidth[1] / max(1, $time - $this->interface->bandwidth[2])) / 1024, 2) . " D " . round(($this->interface->bandwidth[0] / max(1, $time - $this->interface->bandwidth[2])) / 1024, 2) . " kB/s | TPS " . $this->getTPS() . "\x07";
		}
	...
	public function dumpError(){
		...
		$dump = "```\r\n# ModPECore Error Dump " . date("D M j H:i:s T Y") . "\r\n";
		$er = error_get_last();
		$errorConversion = [
			E_ERROR => "E_ERROR",
			E_WARNING => "E_WARNING",
		...
		$dump .= "\r\n\r\n";
		$dump .= "ModPECore version: " . MAJOR_VERSION . " [Protocol " . ProtocolInfo::CURRENT_PROTOCOL . "; API " . CURRENT_API_VERSION . "]\r\n";
